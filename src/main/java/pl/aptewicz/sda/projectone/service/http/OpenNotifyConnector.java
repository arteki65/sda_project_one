...
public class OpenNotifyConnector {
    ...
    public Optional<IssPositionDto> getIssPosition() {
        try {
            final var response = httpClient.send(getIssPositionRequest, HttpResponse.BodyHandlers.ofString());
            if (response.statusCode() == 200) {
                return Optional.ofNullable(jsonMapper.mapIssPositionDtoFromJson(response.body()));
            }
            return Optional.empty();
        } catch (IOException | InterruptedException e) {
            return Optional.empty();
